...
train classifier@(Classifier (KNN _  _)  _) xs ys = KNNKnowledge xs ys classifier

--
classify :: TrainingKnowledge -> ExampleType -> Label
classify knowledge@(KNNKnowledge _ _ (Classifier knn DoubleType)) toClassify =
  LDouble (sum (map labelToDouble $ labelOfClosestNeighbors knowledge toClassify) / fromIntegral (k knn))

classify knowledge@(KNNKnowledge _ _ (Classifier _ IntType)) toClassify = LInt result
  where
...
labelOfClosestNeighbors (KNNKnowledge x y (Classifier knn _)) toClassify =
...
error knowledge@(KNNKnowledge _ _ (Classifier _ IntType)) loss xs ys =
